...
export default function NewDiaryPage() {
  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      if (error || !diary) throw error ?? new Error('日記の保存に失敗しました')
              ...
              {previews.map((src, i) => (
                <div key={i} className="relative aspect-square">
                  <img
                    src={src}
                    alt=""
                    className={`w-full h-full object-cover rounded-xl border-2 transition-all ${thumbnailIndex === i ? 'border-pink-500' : 'border-transparent'}`}
                  />
                  <button
                    onClick={() => setThumbnailIndex(i)}
                    className={`absolute bottom-1 left-1 text-xs px-1.5 py-0.5 rounded-full font-bold transition-all ${thumbnailIndex === i ? 'bg-pink-500 text-white' : 'bg-black/40 text-white'}`}
                  >
