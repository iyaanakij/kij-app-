...
async function searchCastProfiles(hpBase: string, query: string, limit = 5) {
  ...
  // AIが名前を改変できないよう、フォーマット済みの紹介文を付与する
  const formatted = matched.map(p =>
    `■ ${p.name}（${p.age}歳・身長${p.height}cm）\nプロフィール: ${p.profile_url}\n参考: ${p.excerpt}`
  ).join('\n\n')

  return {
    results: matched,
    formatted_candidates: formatted,
    instruction: '上のformatted_candidatesをそのまま使って紹介すること。名前・年齢・身長は一字も変えないこと。',
  }
}
...
4. excerptや説明文の中に出てくる別の人名をキャスト候補として使ってはならない。
5. 名前の補完・推測・創作は厳禁。「〜さん」「〜ちゃん」など名前を変えることも禁止。
...
- 返ってきた results の name フィールドの値だけを使って候補を出す
