...
【オプション制度系の質問の扱い】
━━━━━━━━━━━━━━━━━━━━━━
ロープ・トップレス・混浴は「店のオプション制度」であり、キャスト固有スキルではない。
これらを聞かれたときは「制度説明を主、候補提示を従」で対応する：

【必ずやること】
- まず制度内容・料金・条件を説明する（systemPromptの情報または get_system_info）
- ロープはVIP限定ではなくランジェリー/VIPどちらでも追加可と明言
- 最終的な対応可否はキャスト・当日状況により異なるため電話確認を案内する

【その後の補足（任意）】
- search_cast_profiles を呼んで candidates が返ってきた場合のみ、補足として名前を出す
- candidates が空なら候補提示は行わず、制度説明 + 電話誘導のみで終える
- 候補を出す場合も「プロフィール上で明示している方の一例として」と前置きする

聖水・パンストはキャスト個人の対応差が大きいため、search_cast_profiles で候補を探す。
    ...
    const { text } = await generateText({
      ...
      tools: {
        ...
        search_cast_profiles: tool({
          description: 'キャッシュ済みインデックスを使い、好み・雰囲気・対応プレイに合いそうなキャスト候補を返す。おすすめ女性・可愛い系・綺麗系・清楚系・色気系・初心者向け・聖水・パンストを聞かれたときに使用。ロープ・トップレス・混浴は制度説明を先に行ったうえで、明示候補がある場合のみ補足として呼ぶ。出勤有無や空き状況は返さない。',
          inputSchema: z.object({
