...
function resolveQuery(query: string): { keywords: string[]; requiredOptions: Partial<CastOptions> } {
  ...
  // ロープ・トップレス・混浴・コスプレは system_option 扱い →
  // systemPrompt で制度説明して電話誘導するため、ここでは分類しない
  // カテゴリ指定
...
【オプション制度系の質問の扱い】
━━━━━━━━━━━━━━━━━━━━━━
ロープ・トップレス・混浴・コスプレは「店のオプション制度」であり、キャスト固有スキルではない。
これらを聞かれたときは以下のルールで対応する：

- search_cast_profiles は呼ばない（候補女性を列挙しない）
- systemPromptの料金・コース情報または get_system_info で制度説明を行う
- 最終的な対応可否はキャスト・当日状況により異なるため、電話での確認を案内する
- 「ロープはVIP限定ですか？」→ VIP限定ではなくランジェリー/VIPどちらでも追加可と明言
- ただし「ロープが特に得意な女性は？」のようにキャスト個性として聞かれた場合は
  search_cast_profiles で候補を出してもよい

聖水・パンストはキャスト固有スキルの色が強いため、search_cast_profiles で候補を探す。
    ...
    const { text } = await generateText({
      ...
      tools: {
        ...
        search_cast_profiles: tool({
          description: 'キャッシュ済みインデックスを使い、好み・雰囲気・キャスト固有スキルに合いそうなキャスト候補を返す。おすすめ女性・可愛い系・綺麗系・清楚系・色気系・初心者向け・聖水・パンストなどを聞かれたときに使用。ロープ・トップレス・混浴・コスプレは店のオプション制度のため、このツールを使わずsystemPromptで制度説明する。出勤有無や空き状況は返さない。',
          inputSchema: z.object({
